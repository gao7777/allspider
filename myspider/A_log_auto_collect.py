...
def log_filter(local_date):
    ...
    sql_insert_useraction = "insert into user_action(dialogue_id, user_id, song, action_type, log_date, song_id) VALUES (%s,%s,%s,%s,%s,%s)"
    sql_insert_musiccollection = "insert into babel_recommender_system.music_collection(user_id, song, orider_times) VALUES (%s,%s,%s)"
    ...
    for file in filenames:
        try:
            log_lines = os.popen('zcat %s |grep -i savejob'%(base_hugo_path+file)).readlines()
            log_list.extend(log_lines)
...
def lftptest():
    # obj = subprocess.run(['lftp',' ftpuser:kHVFBW53Wz@47.94.108.110'],shell=True)
