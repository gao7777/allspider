...
def sendMail(mTitle, mContent):
    ...
    receiver = ["gaozhiwei@soundai.com","wangle@soundai.com"]
    # receiver = "ops@soundai.com,caijinsong@soundai.com,chenyong@soundai.com,siwenlei@soundai.com"
    ...
    try:
        ...
        print("send mail error,error is {}".format(e))
...
class check_answer(object):
    def __init__(self, query, skill_name, url=babel_url):
    ...
    def get_answer(self):
        ...
        values = {"merchantId": "7501012293520623844", "clientId": "monitor." + hostName,
                  "deviceId": "monitor." + hostName, "dialogId": dialogId, "ip": "222.67.93.182", "query": "今天天气"}
    ...
    def get_multi_answer(self):
        statuscode=None
        dialogId = str(random.randint(10000, 3456876))
        params = {"merchantId": "7501012293520623844", "query": "呼叫幺五九零幺零三幺二三四", "dialogId": dialogId,
                  "clientId": "monitor." + hostName, "deviceId": "monitor." + hostName, "rate": 1.0}
        headers = {"Content-Type": "application/json"}
        ...
        try:
            ...
            print('get_multi_answer error',e)
            sendMail("【SVK Answer巡检告警】%s Answer异常！" % skill,
                     "multi_answer\n时间：\n %s \n\n输入：\n %s \n\n输出：\n %s \n" % (get_time, json.dumps(params), result))
            statuscode = 0

        return statuscode

    def session_answer(self, list_query):
        statuscode=None
        dialogId = "9999999"
        list_query = list_query
        params = {"dialogId": dialogId, "merchantId": "154", "query": "呼叫幺五九零幺零三幺二三四",
                  "clientId": "monitor." + hostName,
        ...
        try:
            for i in list_query:
                ...
                item = json.loads(result)
                self.write_log(r.text, json.dumps(params))
                if item['domain'] == 'dictionary' or item['domain'] == 'translation' or item['domain'] == 'music' or \
                        item['domain'] == 'mxmusic' or item['domain'] == 'audio':
                    # sendMail("【SVK Answer巡检调试】%s Answer正常！" % skill,"session_answer\n时间：\n %s \n\n请求：\n %s \n\n响应：\n %s \n" % (get_time,json.dumps(params),result))
                    statuscode = 1
                    print('res', result)
                else:
                    print("error statuscode send email domain 偏移")
                    print("【SVK Answer巡检告警】%s 的Domain偏移！" % skill,
                             "session_answer\n时间：\n %s \n\n请求：\n %s \n\n响应：\n %s \n" % (
                                 get_time, json.dumps(params), result))
                    sendMail("【SVK Answer巡检告警】%s 的Domain偏移！" % skill,
                             "session_answer\n时间：\n %s \n\n请求：\n %s \n\n响应：\n %s \n" % (
                                 get_time, json.dumps(params), result))
