...
class XimaSpider(scrapy.Spider):
    ...
    start_urls = ['https://www.ximalaya.com/top/free/','https://www.ximalaya.com/top/rich/','https://www.ximalaya.com/top/subscription/','https://www.ximalaya.com/top/reputation/']
    # start_urls = re_url()
    custom_settings = {
        "DOWNLOAD_DELAY" : 1,
        ...
        "ITEM_PIPELINES": {
            ...
            'myspider.pipelines.XimaremenPipeline': 508,
        }
    }
    ...
    def parse(self, response):
        ...
        category_list = response.xpath("//div[contains(@class,'subbar-wrapper')]/a")
        for i in category_list:
            meta={}
            category_name = i.xpath("./text()").get()
            category_link = "https://www.ximalaya.com"+i.xpath("./@href").get()
            meta['category_name']=category_name
            meta['category_link']=category_link
            yield scrapy.Request(category_link,callback=self.parse_one,meta=meta)
    def parse_one(self,response):
        response= Selector(response)
        title_list = response.xpath("//div[contains(@class,'album-item')]")
        for i in title_list:
            item = dict()
            album_url = "https://www.ximalaya.com"+i.xpath("./a/@href").get()
            href = i.xpath("./a/@href").get()
            album_id = re.search(r"/.*?/(\d+)/",href).group(1)
            album_title = i.xpath("./a/div[contains(@class,'album-info')]/div[@class='title _7N']/text()").get()
            item['album_url'] = album_url
            item['album_title'] = album_title
            item['album_id'] = album_id
            # scr_url = 'https://www.ximalaya.com/revision/play/album?albumId={}&pageNum=2&sort=-1&pageSize=30'

            # response= scrapy.Request(scr_url.format(album_id))
